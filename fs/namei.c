...
/**
 * sb_mknod_capable - check sb for CAP_MKNOD
 * @sb:	super block to which userns CAP_MKNOD should be checked
 *
 * Check sb for CAP_MKNOD to create device node
 *
 * Check mknod for user namespace of sb if corresponding sysctl is set.
 * Otherwise just check global capability for current task. This allows
 * lsm-based guarding of device node creation in non-initial user
 * namespace.
 */
